...
vim.keymap.set('n', '<left>', '<cmd>echo "WARNING: Use h to move"<CR>')
vim.keymap.set('n', '<right>', '<cmd>echo "WARNING: Use l to move"<CR>')
vim.keymap.set('n', '<up>', '<cmd>echo "WARNING: Use k to move"<CR>')
vim.keymap.set('n', '<down>', '<cmd>echo "WARNING: Use j to move"<CR>')
...
require('lazy').setup({
...

  {
    ...
    config = function()
      ...
      oil.setup {
        ...
        -- Toggle Oil in the parent directory
        vim.keymap.set('n', '-', '<CMD>Oil<CR>', { desc = 'Open [O]il in parent directory' }),

        -- Open Oil in a floating window
        vim.keymap.set('n', '<space>-', oil.toggle_float, { desc = 'Open [O]il in a floating window' }),
      }
    end,
  },
...

  {
    ...
    config = function()
      local bufferline_cycle = require 'bufferline-cycle-windowless'

      bufferline_cycle.setup {
    ...
    config = function()
      ...
      cybu.setup {
        position = {
          ...
          vertical_offset = 8, -- vertical offset from anchor in lines
      ...
      require('paint').setup {
        ...
        highlights = {
          ...
          },
          {
            ...
            -- or a function called with buf as param that should return true.
            -- The example below will paint @something in comments with Constant
            filter = { filetype = 'duck' },
            -- -- identifier
            -- pattern = '%s*%-%-%-%s*(@%w+)',
            -- hl = 'Constant',

            -- Highlight function names starting with 'fn' as 'Function'
            pattern = 'fn',
            hl = 'Function',
          },
        },
...
-- Duck Syntax Highlighting
-- vim.cmd [[
--     autocmd BufRead,BufNewFile *.cmu source ~/.config/nvim/lua/custom/plugins/duck.lua
-- ]]
---@class DuckParserInfo : ParserInfo
local parser_config = require('nvim-treesitter.parsers').get_parser_configs()
parser_config.duck = {
  install_info = {
    url = '~/Desktop/dev/repos/personal/private/zig/zz/tree-sitter-duck',
  ...
  -- filetype = 'dk', -- if filetype does not match the parser name
}
